...
/* helper to compute elapsed nanoseconds */
...
int main(int argc, char *argv[])
{
    ...
    struct expected_counts exp;
    struct timespec walk_start, walk_end;   // add this
    long long walk_latency_ns;              // add this
    ...
    /*
     * Time only the kernel-triggered page walk portion.
     */
    if (clock_gettime(CLOCK_MONOTONIC, &walk_start) != 0)
        die("clock_gettime start");

    trigger_walk(start_addr, npages, mode);

    if (clock_gettime(CLOCK_MONOTONIC, &walk_end) != 0)
        die("clock_gettime end");

    walk_latency_ns = diff_ns(&walk_start, &walk_end);

    printf("\nPage-walk trigger latency:\n");
    printf("  %lld ns\n", walk_latency_ns);
    printf("  %.3f us\n", walk_latency_ns / 1000.0);
    printf("  %.3f ms\n", walk_latency_ns / 1000000.0);
